...
export default function ProfileSwitcher() {
  ...
  const createMutation = useMutation({ mutationFn: createProfile, onSuccess: (newProfile) => { void queryClient.invalidateQueries({ queryKey: ['profiles'] }); invalidateHabitsQueries(); setShowCreateForm(false); setNewName(''); setNewEmoji('\u{1F464}'); setNewTarget(10); try { localStorage.setItem(STORAGE_KEY, newProfile.id.toString()); } catch { /* noop */ } router.replace(`/profiles/${newProfile.id}`); } });
